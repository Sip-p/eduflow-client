...
// const [videoLoading, setVideoLoading] = useState(false);  const getCurriculum = async () => {
//     try {
...
//   { isDirectVideo && console.log("Video URL:", currentVideo.videoUrl) }
//   return (
//     <>
//       <style>{`
...
const VideoCourse = () => {
  ...
  // ✅ NEW: separate videoUrl state
  const [videoUrl, setVideoUrl] = useState(null);
  const [videoLoading, setVideoLoading] = useState(false);

  // ✅ NEW: fetch video URL from protected endpoint
  const playLesson = useCallback(async (lesson) => {
    setCurrentVideo(lesson);
    setVideoUrl(null);

    if (lesson.isFree && lesson.videoUrl) {
      // Free lesson — URL already in curriculum
      setVideoUrl(lesson.videoUrl);
      return;
    }

    // Paid lesson — hit the protected endpoint
    setVideoLoading(true);
    try {
      const res = await axios.get(
        `${backendUrl}/api/course/${courseId}/lesson/${lesson._id}/video`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.data.success) setVideoUrl(res.data.videoUrl);
    } catch (err) {
      console.error("Failed to load video:", err.response?.data || err.message);
    } finally {
  ...
  const getCurriculum = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setExpandedChapters(expanded);
        // ✅ CHANGED: was setCurrentVideo(firstLesson)
        if (firstLesson) playLesson(firstLesson);
      }
    } catch (err) {
      console.error("Curriculum fetch error:", err.response?.data || err.message);
    } finally {
  ...
  // ✅ CHANGED: ytUrl now uses videoUrl state, not currentVideo.videoUrl
  const ytUrl = videoUrl ? getYouTubeEmbedUrl(videoUrl) : null;
  const isDirectVideo = currentVideo && !ytUrl;
  ...
  return (
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
          --bg: #0c0c10; --surface: #111116; --surface2: #18181f;
          --border: #25252f; --border2: #2e2e3a; --text: #e4e0d8;
          --text2: #9490a0; --muted: #4a4858; --amber: #e8a87c;
          --amber2: #d4845a; --green: #6db88a;
          --serif: 'Lora', Georgia, serif; --sans: 'Outfit', sans-serif;
        }
        .vc-root { height: 100vh; background: var(--bg); color: var(--text); font-family: var(--sans); display: flex; flex-direction: column; overflow: hidden; }
        .vc-topbar { height: 52px; min-height: 52px; display: flex; align-items: center; justify-content: space-between; padding: 0 24px; background: var(--surface); border-bottom: 1px solid var(--border); position: sticky; top: 0; z-index: 100; gap: 16px; }
        .vc-back { display: flex; align-items: center; gap: 6px; background: none; border: 1px solid var(--border2); color: var(--text2); padding: 6px 14px; border-radius: 6px; font-family: var(--sans); font-size: 12px; font-weight: 500; letter-spacing: 0.03em; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
        .vc-back:hover { border-color: var(--amber); color: var(--amber); }
        .vc-course-name { font-family: var(--serif); font-style: italic; font-size: 15px; color: var(--text); flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; opacity: 0.8; text-align: center; }
        .vc-progress-row { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
        .vc-progress-count { font-family: var(--sans); font-size: 11px; color: var(--muted); font-weight: 500; }
        .vc-progress-track { width: 88px; height: 3px; background: var(--border2); border-radius: 99px; overflow: hidden; }
        .vc-progress-fill { height: 100%; background: var(--amber); border-radius: 99px; transition: width 0.5s ease; }
        .vc-progress-pct { font-family: var(--sans); font-size: 11px; font-weight: 700; color: var(--amber); min-width: 30px; text-align: right; }
        .vc-body { display: flex; flex: 1; overflow: hidden; height: calc(100vh - 52px); }
        .vc-main { flex: 1; overflow-y: auto; display: flex; flex-direction: column; }
        .vc-main::-webkit-scrollbar { width: 4px; }
        .vc-main::-webkit-scrollbar-thumb { background: var(--border2); border-radius: 99px; }
        .vc-video-wrap { background: #000; width: 100%; flex-shrink: 0; }
        .vc-video-placeholder { width: 100%; height: 440px; background: #080810; display: flex; align-items: center; justify-content: center; }
        .vc-meta { padding: 18px 24px; background: var(--surface); border-bottom: 1px solid var(--border); display: flex; align-items: flex-start; justify-content: space-between; gap: 20px; flex-wrap: wrap; flex-shrink: 0; }
        .vc-meta-left { flex: 1; min-width: 0; }
        .vc-lesson-eyebrow { font-family: var(--sans); font-size: 9px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted); margin-bottom: 5px; }
        .vc-lesson-title { font-family: var(--serif); font-size: 22px; font-weight: 600; color: var(--text); line-height: 1.25; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .vc-tags { display: flex; gap: 6px; margin-top: 8px; flex-wrap: wrap; }
        .vc-tag { font-family: var(--sans); font-size: 10px; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; padding: 3px 9px; border-radius: 3px; }
        .vc-tag-free { background: rgba(109,184,138,0.12); color: var(--green); border: 1px solid rgba(109,184,138,0.2); }
        .vc-tag-enrolled { background: rgba(232,168,124,0.10); color: var(--amber); border: 1px solid rgba(232,168,124,0.2); }
        .vc-tag-meta { background: var(--surface2); color: var(--muted); border: 1px solid var(--border); }
        .vc-controls { display: flex; align-items: center; gap: 8px; flex-shrink: 0; flex-wrap: wrap; }
        .vc-btn { padding: 7px 16px; border-radius: 6px; font-family: var(--sans); font-size: 12px; font-weight: 600; letter-spacing: 0.03em; cursor: pointer; transition: all 0.15s; border: 1px solid transparent; white-space: nowrap; }
        .vc-btn-outline { background: none; border-color: var(--border2); color: var(--text2); }
        .vc-btn-outline:hover:not(:disabled) { border-color: var(--text2); color: var(--text); }
        .vc-btn-outline:disabled { opacity: 0.28; cursor: not-allowed; }
        .vc-btn-solid { background: var(--amber); color: #1a0e06; border-color: var(--amber); }
        .vc-btn-solid:hover:not(:disabled) { background: var(--amber2); border-color: var(--amber2); }
        .vc-btn-solid:disabled { opacity: 0.28; cursor: not-allowed; }
        .vc-btn-done { background: rgba(109,184,138,0.12); color: var(--green); border-color: rgba(109,184,138,0.25); cursor: default; }
        .vc-desc { padding: 20px 24px 24px; font-family: var(--serif); font-size: 15px; line-height: 1.85; color: var(--text2); border-bottom: 1px solid var(--border); background: var(--bg); flex-shrink: 0; }
        .vc-desc p { margin-bottom: 0.5em; }
        .vc-tabs { display: flex; background: var(--surface); border-bottom: 1px solid var(--border); padding: 0 24px; flex-shrink: 0; }
        .vc-tab { padding: 13px 20px; font-family: var(--sans); font-size: 11px; font-weight: 600; letter-spacing: 0.1em; text-transform: uppercase; color: var(--muted); background: none; border: none; border-bottom: 2px solid transparent; cursor: pointer; transition: all 0.15s; display: flex; align-items: center; gap: 7px; }
        .vc-tab:hover { color: var(--text2); }
        .vc-tab-active { color: var(--amber); border-bottom-color: var(--amber); }
        .vc-tab-badge { background: var(--amber); color: #1a0e06; font-size: 9px; font-weight: 800; padding: 1px 6px; border-radius: 99px; }
        .vc-tab-body { padding: 28px 24px; background: var(--bg); }
        .vc-assign-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 14px; }
        .vc-assign-card { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; padding: 18px 20px; background: var(--surface); border: 1px solid var(--border); border-radius: 8px; cursor: pointer; transition: all 0.15s; }
        .vc-assign-card:hover { border-color: var(--amber); transform: translateX(4px); }
        .vc-assign-label { font-family: var(--sans); font-size: 9px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--amber); margin-bottom: 4px; }
        .vc-assign-title { font-family: var(--serif); font-size: 17px; color: var(--text); line-height: 1.3; }
        .vc-assign-meta { display: flex; gap: 16px; margin-top: 8px; flex-wrap: wrap; }
        .vc-assign-meta span { font-size: 12px; color: var(--muted); }
        .vc-assign-arrow { font-size: 16px; color: var(--border2); align-self: center; flex-shrink: 0; }
        .vc-chat-wrap { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; overflow: hidden; min-height: 400px; }
        .vc-sidebar { width: 320px; min-width: 320px; background: var(--surface); border-left: 1px solid var(--border); display: flex; flex-direction: column; overflow: hidden; }
        .vc-sidebar-hdr { padding: 16px 20px; border-bottom: 1px solid var(--border); background: var(--surface2); flex-shrink: 0; }
        .vc-sidebar-eyebrow { font-family: var(--sans); font-size: 9px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: var(--muted); margin-bottom: 3px; }
        .vc-sidebar-course { font-family: var(--serif); font-size: 14px; font-style: italic; color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .vc-sidebar-stats { font-family: var(--sans); font-size: 11px; color: var(--muted); margin-top: 4px; }
        .vc-sidebar-scroll { flex: 1; overflow-y: auto; }
        .vc-sidebar-scroll::-webkit-scrollbar { width: 3px; }
        .vc-sidebar-scroll::-webkit-scrollbar-thumb { background: var(--border); }
        .vc-chapter { border-bottom: 1px solid var(--border); }
        .vc-chapter-btn { width: 100%; display: flex; align-items: center; gap: 12px; padding: 12px 20px; background: none; border: none; cursor: pointer; text-align: left; transition: background 0.12s; }
        .vc-chapter-btn:hover { background: rgba(255,255,255,0.025); }
        .vc-chapter-num { font-family: var(--serif); font-size: 20px; font-weight: 400; color: var(--border2); line-height: 1; min-width: 28px; }
        .vc-chapter-info { flex: 1; min-width: 0; }
        .vc-chapter-name { font-family: var(--sans); font-size: 13px; font-weight: 600; color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .vc-chapter-sub { font-size: 11px; color: var(--muted); margin-top: 1px; }
        .vc-chevron { font-size: 9px; color: var(--muted); transition: transform 0.2s; flex-shrink: 0; }
        .vc-chevron-open { transform: rotate(180deg); }
        .vc-lesson { display: flex; align-items: center; gap: 11px; padding: 9px 20px 9px 24px; cursor: pointer; border-left: 3px solid transparent; transition: all 0.12s; background: var(--bg); border-bottom: 1px solid rgba(255,255,255,0.025); }
        .vc-lesson:hover { background: rgba(255,255,255,0.025); }
        .vc-lesson-active { background: rgba(232,168,124,0.07) !important; border-left-color: var(--amber); }
        .vc-lesson-icon { width: 24px; height: 24px; min-width: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 9px; font-weight: 700; transition: all 0.15s; }
        .vc-icon-active { background: var(--amber); color: #1a0e06; }
        .vc-icon-done { background: rgba(109,184,138,0.15); color: var(--green); }
        .vc-icon-default { background: var(--surface2); color: var(--muted); font-size: 10px; }
        .vc-lesson-info { flex: 1; min-width: 0; }
        .vc-lesson-name { font-family: var(--sans); font-size: 12.5px; color: var(--text2); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height: 1.3; font-weight: 400; }
        .vc-lesson-name-active { color: var(--text); font-weight: 600; }
        .vc-lesson-dur { font-size: 10px; color: var(--muted); margin-top: 2px; }
        .vc-free-pill { font-size: 9px; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; background: rgba(109,184,138,0.12); color: var(--green); padding: 2px 6px; border-radius: 3px; flex-shrink: 0; }
        @media (max-width: 1024px) {
          .vc-root { height: auto; overflow: visible; }
          .vc-body { flex-direction: column; height: auto; overflow: visible; }
          .vc-main { overflow: visible; }
        ...
        <div className="vc-body">
          <div className="vc-main">
              ...
              <AnimatePresence mode="wait">
                {currentVideo ? (
                  <motion.div key={currentVideo._id}
                    ...
                    {/* ✅ CHANGED: show loader while fetching URL */}
                    {videoLoading ? (
                      <div className="vc-video-placeholder">
                        <p style={{ color: "var(--muted)", fontFamily: "var(--serif)", fontStyle: "italic" }}>
                          Loading video...
                      ...
                      // ✅ CHANGED: src={videoUrl} not currentVideo.videoUrl
                      <video
                        key={currentVideo._id}
                        src={videoUrl}
                        controls autoPlay
            ...
            {currentVideo && (
              <div className="vc-meta">
                <div className="vc-meta-left">
                  <div className="vc-lesson-eyebrow">Lesson {currentLessonIndex} of {totalLessons}</div>
                  <div className="vc-lesson-title">{currentVideo.title}</div>
                  <div className="vc-tags">
                    <span className={`vc-tag ${currentVideo.isFree ? "vc-tag-free" : "vc-tag-enrolled"}`}>
                      {currentVideo.isFree ? "Free" : "Enrolled"}
                ...
                <div className="vc-controls">
                  {completedLessons.has(currentVideo._id) ? (
                    ...
                    <button className="vc-btn vc-btn-outline" onClick={() => markComplete(currentVideo._id)}>
                      Mark Complete
                    </button>
                  )}
                  {/* ✅ CHANGED: playLesson instead of setCurrentVideo */}
                  <button className="vc-btn vc-btn-outline"
                    disabled={!getPrevLesson()}
                    onClick={() => { const p = getPrevLesson(); if (p) playLesson(p); }}>
                    ← Prev
                  </button>
                  <button className="vc-btn vc-btn-solid"
                    disabled={!getNextLesson()}
                    onClick={() => { const n = getNextLesson(); if (n) playLesson(n); }}>
                    Next →
                  </button>
                </div>
              </div>
            ...
            {currentVideo?.description && (
              <div className="vc-desc" dangerouslySetInnerHTML={{ __html: currentVideo.description }} />
            )}
            ...
            <AnimatePresence mode="wait">
              {activeTab === "chat" && (
                <motion.div key="chat" className="vc-tab-body"
                  initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                  <div className="vc-chat-wrap"><GroupChat courseId={courseId} /></div>
                </motion.div>
              )}
              {activeTab === "assignments" && (
                <motion.div key="assignments" className="vc-tab-body"
                  ...
                  <div className="vc-assign-grid">
                    {assignments.map((a, idx) => (
                      <motion.div key={a._id} className="vc-assign-card"
          ...
          <div className="vc-sidebar">
            ...
            <div className="vc-sidebar-scroll">
              {curriculum.length > 0 ? curriculum.map((chapter, ci) => (
                ...
                <p style={{ textAlign: "center", color: "var(--muted)", marginTop: 48, fontSize: 14, fontFamily: "var(--serif)", fontStyle: "italic" }}>
                  No lessons available
                </p>
              )}
            </div>
